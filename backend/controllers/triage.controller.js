...
import { SANJEEVANI_SYSTEM_PROMPT } from '../config/systemPrompt.js';
...
function buildUserMessage({ narrative, language, biometrics, bodyRegion }) {
  ...
  if (biometrics) {
    ...
    if (b.bpSystolic && b.bpDiastolic) bLines.push(`Blood Pressure: ${b.bpSystolic}/${b.bpDiastolic} mmHg`);
...
async function callMedGemma(userMessage) {
  ...
  if (!response.ok) throw new Error(`HF API error ${response.status}: ${responseText.slice(0, 300)}`);

  let data;
  try { data = JSON.parse(responseText); }
  catch { throw new Error(`HF API returned non-JSON: ${responseText.slice(0, 200)}`); }
...
async function callGemini(userMessage) {
  ...
  if (!geminiKey.startsWith('AIza')) {
    throw new Error(`GEMINI_API_KEY appears invalid (should start with "AIza", got "${geminiKey.slice(0, 8)}..."). Get a valid key at https://aistudio.google.com/app/apikey`);
  }
...
export const runTriage = async (req, res) => {
  try {
    const { narrative, language, biometrics, bodyRegion } = req.body;
    ...
    try {
      ...
      const mock = getMockData(narrative, forceRed);
      return res.json({ ...mock, ambulance_triggered: forceRed });
    }
    ...
    const finalResult = { ...parsed, _source: source, ambulance_triggered: parsed.Severity_Color === 'RED' };
